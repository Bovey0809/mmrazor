...
class QConfigHandler():
    ...
    def __init__(self, qconfig: Union[Dict, Config]):
        ...
        w_is_per_channel = 'PerChannel' in w_observer.__name__
        a_is_per_channel = 'PerChannel' in a_observer.__name__
    ...
    def convert(self):
        ...
        return QConfig(weight=self.w_fake_quant, activation=self.a_fake_quant)
    ...
    def replace_fakequant(fake_quant_org: FakeQuantize,
        ...
        pop_keys = [k for k in observer_kwargs.keys() if k not in retain_args]
        for k in pop_keys:
